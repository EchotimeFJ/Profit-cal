...
- [x] Task 3: 前端历史净值图接入自动分钟级数据
  - [x] SubTask 3.1: 更新 `PortfolioHistoryPoint` 类型，支持 `timestamp` 字段并兼容 `date`
  - [x] SubTask 3.2: 更新 Dashboard 历史接口调用，切换 `1日/3日/7日` 时请求后端对应 range
  - [x] SubTask 3.3: 更新图表组件，移除前端本地范围裁剪，使用后端返回点位渲染金额 Y 轴和时间 X 轴
  - [x] SubTask 3.4: 移除“手动刷新生成每日快照”文案，改为“系统自动记录组合历史”
  - [x] SubTask 3.5: 保持移动端不横向溢出，点位为空或单点时展示稳定空态/坐标框架

- [x] Task 4: CI、回归与部署说明
  - [x] SubTask 4.1: 将新增后端测试、脚本和服务加入 CI unittest 与 compileall
  - [x] SubTask 4.2: 执行完整后端回归、前端构建、空白检查
  - [x] SubTask 4.3: 更新现有说明文档，给出 cron 每分钟执行采集脚本的部署命令
  - [x] SubTask 4.4: 最终复核数据安全：不修改旧资产、交易、提醒、日级历史数据
